...
namespace Mobius.SpotfireComOps
{
	...
	public class Lex
	{
		...
		public static void AppendToList(
				...
				string item)
		{
			AppendIfNotBlank(ref dest, separator);
			dest += item;
			return;
		}
		...
		public static void AppendIfNew(
			ref string dest,
			string source,
			string separator)
		{
			...
			Lex.AppendIfNotBlank(ref dest, separator);
			dest += source;
			return;
		}
		...
		public static void AppendIfNotBlank(
			ref string dest,
			string separator)
		{
			...
			string substringAfter)
		{
			...
			//string pattern = @".*" + Regex.Escape(substringBefore) + "(.*)" + Regex.Escape(substringAfter) + ".*"; // (can match too much)
...
\G     Specifies that the matches must be consecutive, without any intervening non-matching characters.  
       */
		...
		//	return NumberEx.DoubleTryParseExact(tok, out d); // more accurate check for string that is a double
		//}
