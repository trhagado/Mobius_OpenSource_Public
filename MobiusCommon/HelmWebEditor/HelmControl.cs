...
{
	public partial class HelmControl : DevExpress.XtraEditors.XtraUserControl
	{
		[DefaultValue(HelmControlMode.BrowserViewOnly)]
		public HelmControlMode HelmMode { get => _helmMode; set => _helmMode = value; }
		HelmControlMode _helmMode = HelmControlMode.BrowserViewOnly;

		public bool BitmapMode => (_helmMode == HelmControlMode.OffScreenBitmap); // rendered off-screen and returned as a bitmap that can then be used anywhere
		public bool SvgMode => (_helmMode == HelmControlMode.OffScreenSvg); // rendered off-screen and returned as SVG that can be used anywhere 
		public bool ViewMode => (_helmMode == HelmControlMode.BrowserViewOnly); // view helm in browser without edit controls
		public bool EditMode => (HelmMode == HelmControlMode.BrowserEditor); // show helm in editor
		...
		/// <summary>
		/// Initialize the on-screen and/or off-screen rendering controls based on the selected HelmControlMode
		/// </summary>

		public void InitializeControl()
		{
			if (HelmMode == HelmControlMode.OffScreenBitmap)
			{
			...
			else if (HelmMode == HelmControlMode.OffScreenSvg)
			{
				if (OffScreenBrowserMx != null) return;
			...
			if (Debug) DebugLog.Message("HelmControl initialized for Mode: " + HelmMode + IdText);
			return;
		}
		...
		void SetupOnScreenBitmapImageControl()
		{
			...
			OnScreenImageCtl.Click += new EventHandler(HelmControl_Click); // allow edit of controls contents
			OnScreenImageCtl.DoubleClick += new EventHandler(HelmControl_DoubleClick);
...
/// <returns></returns>
		...
		public string GetSvg()
		{
			...
			if (HelmMode != HelmControlMode.OffScreenSvg)
				throw new Exception("Expected HelmMode == HelmControlMode.OffScreenSvgHelmControlMode but HelmMode is " + HelmMode);
		...
		private void HelmControl_Click(object sender, EventArgs e)
		{
		...
		private void HelmControl_DoubleClick(object sender, EventArgs e)
		{
			return;
		}
		...
		private void HelmControl_SizeChanged(object sender, EventArgs e)
		{
			if (WinFormsBrowserMx == null && OffScreenBrowserMx == null) return; // just return if nothing in HELM control yet

			if (Debug) DebugLog.Message("HelmControl_SizeChanged: " + this.Size.Width + ", " + this.Size.Height);
