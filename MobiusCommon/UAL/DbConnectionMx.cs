...
namespace Mobius.UAL
{
	...
	public class DbConnectionMx
	{
		...
		public Schema LastRootSchema = null; // last root schema that was used when getting this connection
		...
		public static string TestKeyConnections()
		{
			...
			foreach (DataSourceMx source in DataSourceMx.DataSources.Values)
			{
				...
				catch (Exception ex)
				{
					...
					foreach (Schema schema in DataSourceMx.Schemas.Values)
					{
			...
			string forcedSchemaName)
		{
			...
			Dictionary<string, Dictionary<string, Schema>> connDict = GetDataSources(sql, out rootSource, out rootSchema);
			...
			foreach (string schemaName in snobl)
			{
				...
				Schema schema = DataSourceMx.Schemas[schemaName];
			...
			Dictionary<string, Dictionary<string, Schema>> connDict)
		{
			...
			foreach (Dictionary<string, Schema> sd in connDict.Values)
			{
				foreach (Schema s in sd.Values)
				{
		...
		public static Dictionary<string, Dictionary<string, Schema>> GetDataSources(
			string sql,
			out DataSourceMx rootSource,
			out Schema rootSchema)
		{
			//if (Lex.Contains(sql, "_moltable")) sql = sql; // debug

			Dictionary<string, Dictionary<string, Schema>> sourceDict =
				new Dictionary<string, Dictionary<string, Schema>>(StringComparer.OrdinalIgnoreCase);
			...
			while (true) // parse the SQL looking for schema names
			{
				...
				if (!sourceDict.ContainsKey(sourceName)) // first entry for source?
					sourceDict[sourceName] = new Dictionary<string, Schema>(StringComparer.OrdinalIgnoreCase); // create schema dict
		...
		public static void ThrowSpecificConnectionOpenException(
				...
				string schemaName)
		{
			Schema schema = DataSourceMx.Schemas[schemaName.ToUpper()];
		...
		/// <summary>
		/// Get the name of a datasource associated with a schema
		/// </summary>
		/// <param name="schemaName"></param>
		/// <returns></returns>

		public static string GetSchemaDataSourceName(string schemaName)
		{
			if (Lex.IsUndefined(schemaName)) return ""; // schema not defined

			schemaName = schemaName.ToUpper();
			if (!DataSourceMx.Schemas.ContainsKey(schemaName)) return ""; // unknown schema

			Schema schema = DataSourceMx.Schemas[schemaName];
			return schema.DataSourceName.ToUpper();
		}
